...
function toggleLoading(isLoading) {
  ...
  searchIcon.removeChild(searchIcon.childNodes[0]);

  if (isLoading == true) {
    searchIcon.setAttribute("class", "fas fa-hourglass-half");
    searchIcon.appendChild(createText(" Loading...")); //see top of ui.js for createText()
    searchButton.disabled = true;
  } else if (isLoading == false) {
    searchIcon.setAttribute("class", "fas fa-search");
    searchIcon.appendChild(createText(" Search"));
    searchButton.disabled = false;
  }

  /*if (isLoading == true) {
    searchButton.setAttribute("class", "fas fa-hourglass-half");
    searchButton.removeChild(searchButton.childNodes[0]);
    searchButton.appendChild(createText(" Loading...")); //see top of ui.js for createText()
    searchButton.disabled = true;
  } else if (isLoading == false) {
    searchButton.setAttribute("class", "fas fa-search");
    searchButton.removeChild(searchButton.childNodes[0]);
    searchButton.appendChild(createText(" Search"));
    searchButton.disabled = false;
  }*/
}
